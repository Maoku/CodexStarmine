...
1. **`reference/Box3D` が存在しない。** PLAN.md は物理エンジンとして参照しているが、リポジトリに `reference/` ディレクトリ自体がない。
   - 本計画では、星の弾道は自作積分器で実装する（花火の星は衝突しない質点であり、剛体エンジンは本質的に不要なため）。
   - Box3D が提供された場合の想定用途: 打上筒・玉殻の演出、地形との相互作用など補助的な範囲。提供時に組込み箇所を再検討する。
2. PLAN.md が参照する `Docs/REASERCH.md` は `Docs/RESEARCH.md` の誤記と判断し、後者を根拠資料とした。
3. 音楽ファイルの入力方式（ローカルファイル選択のみか、内蔵曲も持つか）は Phase 7 着手前に決める。
