...
- 完了日: 2026-07-16
- `PatternRecipe` を追加し、円／ハートの形状、12〜240点の密度、15〜95%の大きさ、15度刻みの回転、断面を決定的な座標へ解決する正本にした。v4 resolverも同じレシピを使用する。
- 型物の形状を中央ワークベンチへ、大きさ、密度、回転を右ペインへ追加した。生成点は参照表示のままにし、点選択、ドラッグ、個別削除、個別星置換を許可しない。
- 手動の便利配置へ `置換 / 追加` を追加し、円36点、ハート44点を現在断面へ等間隔生成する。生成全体を1履歴として記録し、直後のUndo/Redoで全点を戻せるようにした。
- 手動点の移動、削除、星置換をv4の1点へ直接適用し、既定／型物では同じイベント経路がデータを変更しないようauthoring modeでガードした。
- ブラウザで型物72点への密度変更と編集可能点0件、手動の円36点→ハート追加80点→Undo 36点→ハート置換44点、1点削除43点→Undo 44点を確認した。

### Renewal2 Phase 6

- 完了日: 2026-07-16
- 星の広がりバルーンを部品皿のDOM外にある固定オーバーレイへ移し、アンカーとビューポートから上下反転と左右クランプを計算するようにした。四隅の座標単体テストを追加した。
- バルーンをモーダルなdialogとして扱い、開いた直後のフォーカス、Tabの閉じる操作への保持、Escape／外側クリック／閉じるボタン、起点ボタンへのフォーカス復帰を実装した。
- 右ペインを内部スクロールする設定領域と固定の簡易確認へ分割した。簡易確認の150msデバウンスと軽量2D描画は維持した。
- 1280×720で外側スクロールなしに断面切替、便利配置、簡易確認、保存、湖面確認が同時に見えることを確認した。部品皿下端の星バルーンは上側へ反転し、部品皿内にネストされない。
- 390×844で横流出のない中央ワークベンチ、常設フッターの保存／湖面確認、左右ドロワーからの星配置と設定、長い型物設定でも固定表示される簡易確認を確認した。
