...
### Renewal Phase 4: 統合編集ワークベンチ（完了: 2026-07-16）

- [x] 制作工程ペインを削除し、レイヤーと部品皿を拡張する。
- [x] 半球・断面図・型物の表示タブを削除する。
- [x] 玉皮、配置面、点群を同じ正規化座標の常設ワークベンチへ統合する。
- [x] 緯度・経度の4区画選択と配置面の回転を実装する。
- [x] 円形、ハート、手動配置を実装する。
- [x] 仮想星の長押しバルーンとキーボード代替操作を実装する。
- [x] 編集画面内へ小窓の簡易確認を実装する。
- [x] ドラッグ操作を1件のUndo履歴へまとめる。
- [x] デスクトップとモバイルドロワーを実装する。
- [x] 工房の木製作業台、実用パネル、限定的な金色アクセントへスタイルを整理する。

完了基準: 表示タブなしで星を配置面へ置き、便利配置を個別編集し、小窓の簡易確認、Undo/Redo、保存ができる。

**完了確認**:

- `src/ui/craft/IntegratedCraftEditor.ts` を画面の実入口へ接続し、制作工程ペインと表示タブを持たない3領域の統合編集画面へ置き換えた。左側は拡張レイヤーと部品皿、中央は常設ワークベンチ、右側は意図レベルの設定と小窓へ整理した。
- `src/ui/craft/IntegratedPlacementWorkbench.ts` に、半径1の正規化座標、緯度4区画、経度4区画、配置面回転、円形／ハートの決定的点群、手動追加・移動・削除を実装した。球面の個別編集は既存 `StarPointOverride`、型物は既存 `PatternPoint` を使い、保存形式を増やしていない。
- 点ドラッグ中はDOM上の位置だけを追従させ、`pointerup` 時に `CraftDocumentStore.update` を1回だけ実行する構成とした。実ブラウザで移動前、移動後、Undo後の点座標を比較し、Undo 1回で元の座標へ戻ることを確認した。
- `src/ui/craft/StarLibraryPanel.ts` に450ms長押し、8px移動取消、pointer解放／cancel、`Escape`、`Space` と専用 `広がりを見る` 操作を接続した。バルーンは単一の `VirtualStarPreset` だけを2D抽象表示し、`FireworkSystem` は使用していない。
- `src/render/preview/ApproximateSpreadRenderer.ts` と `src/ui/craft/InlineDiagnosticPreview.ts` に、全レイヤーを集約した色点と広がりリングの小窓を追加した。150msデバウンス、自動再生、一時停止、先頭へを実装し、モデルに本番位置・速度座標が含まれないことを単体テストで固定した。
- 900px以下ではレイヤー／部品皿と設定／簡易確認を左右ドロワーへ移し、中央ワークベンチと下部操作を常設した。工房の木と暗色パネルを基調にし、金色は選択、フォーカス、主操作へ限定した。`prefers-reduced-motion` では小窓アニメーションとドロワー遷移を停止する。
...
- [x] 編集画面に制作工程ペインが存在しない。
- [x] レイヤーと部品皿が旧レイアウトより広い。
- [x] 半球・断面図・型物の表示タブが存在しない。
- [x] 緯度・経度の粗い区画から配置面を選び、星を配置できる。
- [x] 円形、ハート、手動の配置を使える。
- [x] 仮想星の長押しとキーボード操作で抽象プレビューを開ける。
