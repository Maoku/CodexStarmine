...
- [ ] `AppFlowController` と `AppScreen` を実装する。
- [ ] モード選択画面を実装する。
- [ ] 画面ごとのmount/unmountと戻る操作を実装する。
- [ ] 未保存編集の離脱ガードを実装する。
- [ ] 現行モードタブを新しい入口へ置き換える。

完了基準: モード選択から仮の花火棚とfreeコンテキストへ遷移し、戻る操作が一貫する。

### Renewal Phase 2: 花火棚と初期設定

- [ ] 保存作品のカード一覧、検索、並替、空状態を実装する。
- [ ] 新規、編集、削除確認を花火棚へ集約する。
- [ ] 大きさと菊／牡丹／白紙を選ぶ初期設定画面を実装する。
- [ ] 初期設定から一時draftを作り、取消時には保存しない。
- [ ] 玉外装／断面の抽象サムネイル生成を実装する。
- [ ] 木製作業棚、ラベル差し、収納引出しを持つ簡素な工房スタイルを実装する。
...
- [ ] 起動時にモード選択が表示される。
- [ ] 「花火を作る」から花火棚へ遷移する。
- [ ] 花火棚で新規、編集、削除、削除取消ができる。
- [ ] 新規作成の設定項目は大きさとテンプレートだけである。
- [ ] 編集画面に制作工程ペインが存在しない。
- [ ] レイヤーと部品皿が旧レイアウトより広い。
...
- [ ] 花火棚と編集画面が工房を基調とし、過剰な飾り罫、柄、発光を使わない。
- [ ] v2作品を失わずv3へ移行し、v2キーが残る。
- [ ] 既存6プリセット、完成打上、フリー鑑賞が回帰しない。
- [ ] `npm run lint`、`npm run test:run`、`npm run build` が成功する。
