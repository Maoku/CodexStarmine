...
塗りつぶしを除いた版は、円形グリッドの各セルを8近傍と比較し、次のどちらかを満たすセルだけ残す。

- 近傍に `.` がある外形セル
- 近傍色との知覚色差がしきい値以上の内部境界セル

参照実装ではRGB差を輝度係数 `0.30 / 0.59 / 0.11` で重み付けし、色差100以上を内部エッジとした。前髪など淡いピンク同士の境界には色差30以上の補助判定を加え、境界の暗い側だけを残して細い線にする。これにより肖像は1,257点から548点へ減り、外周96点を含めても644星に収まる。

```ts
const distance = Math.sqrt(
  redDifference ** 2 * 0.3 +
    greenDifference ** 2 * 0.59 +
    blueDifference ** 2 * 0.11,
);
const isEdge = neighbor === "." || distance >= 100;
```

髪の補助判定で明暗両側を残すと二重線になり、面が再び埋まって見えやすい。現在セルの輝度が近傍より低い場合だけ残すことで、前髪や毛束の内部境界を1列の光跡として維持する。外形判定は色差に関係なく常に残す。

「七夕のまお・光輪郭」では、白い初光から元のパレット色へ変わる4段階発光と0.22秒の短い尾を使う。長い尾は隣接する線を再び面に見せるため避ける。通常版と同様に `PatternStarLayer` を使い、schema v4移行後も固定座標を維持する。
...
3件を `FIREWORK_PRESETS` へ追加する。既存見本の順序を前提にする回帰テストがあるため、凍結された既存プリセット群の途中へ挿入せず、配列後方へ追加する。
...
- 線画版は肖像548点、外周込み644星で、肖像点数が通常版の半分未満である。
