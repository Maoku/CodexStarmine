...
### 3.2 点群抽出アルゴリズム（2026-07-17改訂: 洪水充填背景 + 輪郭優先予算）

被写体の面を敷き詰めるのではなく、被写体の外形輪郭を最優先で密にたどり、残りを内部特徴（目など）へ充てる。

1. **縮小**: 最長辺128pxへ縮小してImageDataを取得する（処理量の上限を固定）。
2. **背景推定**: 外周1pxの画素から背景色を推定（中央値）。透過画像はアルファ0を背景とみなす。
3. **背景の洪水充填**: 各画素の被写体強度 = 背景色とのRGB距離 × アルファ（透過画像はアルファ）。画像外周から強度がしきい値未満の画素を4連結でたどった領域だけを背景とする。被写体に囲まれた背景色領域（白背景上の白い服など）は被写体側に残る。
4. **分離ノイズの除外**: 被写体の連結成分ごとに強度合計を採点し、最高成分の15%未満の成分（壁の陰影の帯、切れた小片など）は配置対象から除外する。
5. **シルエット抽出**: 残った被写体画素のうち、4近傍に背景がある画素を外形輪郭とする。画像端は背景とみなさず、クロップ線に点を置かない。
...
8. **予算配分と決定的サンプリング**: 目標点数（既定240、上限240）の62%をシルエットへ優先配分し、輪郭長 ÷ 予算の間隔で等間隔に貪欲選択する。残り予算を特徴エッジのスコア降順に、既選択点との最小距離を保って選ぶ。特徴が少なければ余剰をシルエットへ戻してさらに密にする。距離を満たせない場合は段階的に緩和する。ソートはスコア→画素インデックスの順で安定させる。
9. **失敗判定**: 採用点が8未満なら「被写体を検出できませんでした」としてレイヤーを変更しない。
...
- 抽出モード（輪郭／敷き詰め／網点）の選択UI。抽出は輪郭・特徴点ベースのみとする。
